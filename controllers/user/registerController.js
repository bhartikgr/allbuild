...
exports.getallsharedCodeByCompany = (req, res) => {
  var data = req.body;
  db.query(
    "SELECT shared_discount_code.*,discount_code.percentage FROM shared_discount_code join discount_code on discount_code.code = shared_discount_code.discount_code where shared_discount_code.shared_id =? order by id desc",
    [data.user_id],
    async (err, results) => {
      if (err) {
        return res
